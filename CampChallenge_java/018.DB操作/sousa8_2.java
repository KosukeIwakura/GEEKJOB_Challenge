 ...
 *
 */
public class sousa8_2 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            // 情報の取り出し　入力された文字をmoziに代入 
            String mozi = request.getParameter("name");

            // 表示 
            if (mozi != null) {

                out.print("検索ワード「" + request.getParameter("name")
                        + "」<br><br>");
            ...
            // 検索フォーム（sousa8-1.jsp）で受けた情報を参照するのでselect 
            db_st = db_con.prepareStatement("select*from profiles where name like ?");

            // 「?」にmoziを指定　"%mozi%"にするとmoziという文字が含まれていることになってしまう 
            db_st.setString(1, "%" + mozi + "%");
